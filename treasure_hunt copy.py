...
class DofusWindow() :
    ...
    def waitForCoordsChange(self, delay=1, timeout=30) :
        self.lastPos = self.pos
        self.proxy = Proxy(timeout)
        print("Fin de proxy")
        print("From another pov : ", self.proxy.changed)
        time.sleep(delay)
        if self.proxy.changed :
            self.pos = self.proxy.pos
            print("Pos so are : ", self.pos)
        return self.proxy.changed
        cnt = 0
        while cnt < timeout :
            time.sleep(delay)
    ...
    def moveWithConfirmation(self, direction, forceMove=True) :
